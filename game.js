...
function resize(){const rect=wrap.getBoundingClientRect(),mobile=matchMedia('(pointer:coarse)').matches;currentDpr=Math.min(mobile?1.35:1.6,window.devicePixelRatio||1);width=Math.max(280,Math.round(rect.width));height=Math.max(360,Math.round(rect.height));canvas.width=Math.max(1,Math.round(width*currentDpr));canvas.height=Math.max(1,Math.round(height*currentDpr));canvas.style.width=width+'px';canvas.style.height=height+'px';ctx.setTransform(currentDpr,0,0,currentDpr,0,0);rebuildWalls();dropX=clamp(dropX,30,width-30);updateAim()}
...
function mergePair(a,b){if(!a||!b||a.mergeLock||b.mergeLock||a.fruitTier!==b.fruitTier||a.fruitTier>=TIERS.length-1)return;a.mergeLock=b.mergeLock=true;merging.add(a.id);merging.add(b.id);const next=a.fruitTier+1,x=(a.position.x+b.position.x)/2,y=(a.position.y+b.position.y)/2,vx=(a.velocity.x+b.velocity.x)*.22,vy=(a.velocity.y+b.velocity.y)*.12;queueMicrotask(()=>{if(!fruits.includes(a)||!fruits.includes(b))return;removeFruit(a);removeFruit(b);const c=makeFruit(next,x,y,{vx,vy});Body.setAngularVelocity(c,(a.angularVelocity+b.angularVelocity)*.2);addScore(TIERS[next].score,'merge');pulseScore();sendEvent('fruit_merged',{fromTier:next-1,toTier:next,points:TIERS[next].score});sendState('merge')})}
...
function updateNextUI(){setChip($('nextFruit'),nextTier);dropTier=nextTier;setChip($('dropPreview'),dropTier);updateAim()}
function updateAim(){const t=TIERS[dropTier],r=t.r;dropX=clamp(dropX,r+3,width-r-3);$('aimLine').style.left=dropX+'px';const preview=$('dropPreview');preview.style.left=dropX+'px';preview.style.width=Math.max(22,r*1.2)+'px';preview.style.height=Math.max(22,r*1.2)+'px'}
function drop(tier=null,x=dropX,source='player'){if(gameOver||paused||!canDrop)return false;const chosen=tier===null?dropTier:clamp(Number(tier)||0,0,TIERS.length-1),t=TIERS[chosen];makeFruit(chosen,clamp(x,t.r+3,width-t.r-3),Math.max(18,t.r+4));canDrop=false;setTimeout(()=>{canDrop=true},300);if(tier===null){nextTier=randomSpawn();updateNextUI()}sendEvent('fruit_dropped',{tier:chosen,source});sendState('drop');return true}
function resetGame(){fruits.slice().forEach(removeFruit);score=0;$('score').textContent='0';nextTier=randomSpawn();updateNextUI();gameOver=false;paused=false;dangerSince=0;hideMessage();sendEvent('game_reset');sendState('reset')}
...
function checkDanger(now){if(gameOver||paused||now-lastDangerCheck<120)return;lastDangerCheck=now;const stamp=Date.now(),risky=fruits.some(b=>stamp-b.spawnedAt>1300&&b.position.y-TIERS[b.fruitTier].r<88&&Math.abs(b.velocity.x)+Math.abs(b.velocity.y)<1.45);if(risky){if(!dangerSince)dangerSince=stamp;if(stamp-dangerSince>2300)endGame()}else dangerSince=0}
function drawFruit(b){const t=TIERS[b.fruitTier],sprite=fruitSprites[b.fruitTier],r=t.r,pad=(sprite.width/2)-r,half=r+pad;ctx.save();ctx.translate(b.position.x,b.position.y);ctx.rotate(b.angle);ctx.drawImage(sprite,-half,-half,half*2,half*2);ctx.restore()}
function draw(){ctx.clearRect(0,0,width,height);for(let i=0;i<fruits.length;i++)drawFruit(fruits[i])}
function tick(now){const elapsed=Math.min(50,Math.max(0,now-last));last=now;if(!paused&&!gameOver){accumulator+=elapsed;let steps=0;while(accumulator>=FIXED_STEP&&steps<MAX_SUBSTEPS){Engine.update(engine,FIXED_STEP);accumulator-=FIXED_STEP;steps++}if(steps===MAX_SUBSTEPS&&accumulator>FIXED_STEP)accumulator=0}else accumulator=0;checkDanger(now);draw();requestAnimationFrame(tick)}
function stateSnapshot(scope='game'){return{scope,gameId:GAME_ID,version:VERSION,score,gameOver,paused,nextTier,nextFruit:TIERS[nextTier].id,fruitCount:fruits.length,highestTier:fruits.reduce((m,b)=>Math.max(m,b.fruitTier),0),rules:panelRules.length,transport:session?.getTransport?.()||'offline'}}
