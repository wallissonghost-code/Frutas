(()=>{'use strict';
const C=window.FRUTAS_CONFIG||{},SEEN_KEY=C.seenCommandsKey||'frutas-live-seen-v1',seen=new Map(),queues=new Map();
...
async function run(data,executor){cleanup();const action=String(data?.action||data?.command||''),id=eventId(data);if(id&&seen.has(id))return{ok:true,duplicate:true};if(id){seen.set(id,Date.now());persist()}const mode=policy(action);if(mode!=='queue')return{ok:!!(await executor(data)),duplicate:false};const prev=queues.get(action)||Promise.resolve();const next=prev.catch(()=>{}).then(()=>executor(data));queues.set(action,next.finally(()=>{if(queues.get(action)===next)queues.delete(action)}));return{ok:!!(await next),duplicate:false}}
function clear({persisted=false}={}){seen.clear();queues.clear();if(persisted){try{localStorage.removeItem(SEEN_KEY)}catch{}}else persist()}
load();cleanup();
